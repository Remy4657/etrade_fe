const WhoAreData = [
    {
        title: "Chúng tôi là ai",
        text: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores, saepe."
    },
    {
        title: "Chúng tôi làm gì",
        text: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores, saepe."
    },
    {
        title: "Chúng tôi giúp đỡ như thế nào",
        text: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores, saepe."
    },
    {
        title: "Tạo nên câu chuyện thành công",
        text: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores, saepe."
    },
]
const WhoAreGallery = ["/images/about/about-04.png", "/images/about/about-06.png", "/images/about/about-05.png", "/images/about/about-07.png"]

const AboutStore = {
    thumbnail: "/images/product/product-07.webp",
    subtitle: "Về Cửa hàng",
    subtitleIcon: "far fa-shopping-basket",
    title: "Mua sắm trực tuyến bao gồm cả việc mua sắm và bán hàng qua internet.",
    higlightLine: "Salesforce B2C Commerce giúp bạn tạo ra trải nghiệm thương mại điện tử thống nhất và thông minh — cả trực tuyến và trong cửa hàng.",
    description: [
        "Củng cố đội ngũ bán hàng với giải pháp phù hợp với ngành để hỗ trợ các nhà sản xuất trong quá trình số hóa và thích nghi nhanh hơn với thị trường và khách hàng thay đổi bằng cách tạo ra các cơ hội kinh doanh mới.",
        "Salesforce B2B Commerce cung cấp trải nghiệm mua sắm trực tuyến liền mạch và tự phục vụ cho người mua, đáp ứng mọi nhu cầu B2B."
    ]
}

const AboutAchievment = [
    {
        icon: "/images/about/shape-01.png",
        title: "40.000+ Khách hàng hài lòng",
        text: "Củng cố đội ngũ bán hàng của bạn với giải pháp phù hợp với ngành để hỗ trợ."
    },
    {
        icon: "/images/about/shape-02.png",
        title: "16 Năm Kinh nghiệm",
        text: "Củng cố đội ngũ bán hàng của bạn với giải pháp phù hợp với ngành để hỗ trợ."
    },
    {
        icon: "/images/about/shape-03.png",
        title: "12 Giải thưởng đã giành",
        text: "Củng cố đội ngũ bán hàng của bạn với giải pháp phù hợp với ngành để hỗ trợ."
    }
]

const AboutFeatures = [
    {
        subtitle: "Tính năng #01",
        title: "Các giải pháp cùng làm việc",
        text: "Xuất bản trang web thương mại điện tử của bạn nhanh chóng với công cụ tạo cửa hàng dễ sử dụng của chúng tôi— không cần viết mã. Di chuyển các mặt hàng từ hệ thống điểm bán hàng hoặc chuyển đổi kênh Instagram của bạn thành trang web mua sắm và bắt đầu bán ngay. Square Online phù hợp với mọi loại doanh nghiệp—bán lẻ, nhà hàng, dịch vụ.",
        thumbnail: "/images/product/product-08.webp"
    },
    {
        subtitle: "Tính năng #02",
        title: "Các giải pháp cùng làm việc",
        text: "Xuất bản trang web thương mại điện tử của bạn nhanh chóng với công cụ tạo cửa hàng dễ sử dụng của chúng tôi— không cần viết mã. Di chuyển các mặt hàng từ hệ thống điểm bán hàng hoặc chuyển đổi kênh Instagram của bạn thành trang web mua sắm và bắt đầu bán ngay. Square Online phù hợp với mọi loại doanh nghiệp—bán lẻ, nhà hàng, dịch vụ.",
        thumbnail: "/images/product/product-09.webp"
    }

]


export { WhoAreData, WhoAreGallery, AboutFeatures, AboutStore, AboutAchievment };

